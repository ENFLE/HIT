...
DEFAULT_SETTINGS = {
    ...
    "credit_price_ton": "0.01",
    "ton_packages": "0.01,0.05,0.1,0.5,1,5",
...
MSG_TEMPLATES: dict[str, dict] = {
    ...
    "msg_symbol_prompt": {
        "label": "🏷 پیام «انتخاب نماد»",
        "vars": ["trait_list"],
        "default": "🏷 نمادهای (Symbol) موجود رو انتخاب کن:{trait_list}",
    },
    "msg_result_summary": {
    ...
    "msg_symbol_confirm_min_one": {
        "label": "⚠️ هشدار انتخاب حداقل یک نماد",
        "vars": [],
        "default": "حداقل یک نماد انتخاب کن.",
    },
    "msg_symbol_all_disabled_by_others": {
        "label": "⚠️ هشدار تداخل «انتخاب همه نماد» با مدل/بک‌گراند",
        "vars": [],
        "default": "چون «انتخاب همه» مدل یا بک‌گراند فعاله، نمی‌تونی «انتخاب همه نماد» رو هم بزنی.",
    },
    "msg_scan_starting": {
    ...
    "msg_all_symbols_label": {
        "label": "🏷 برچسب «همه نمادها» در خلاصه نتیجه",
        "vars": [],
        "default": "همه نمادها",
    },
    "msg_scan_result_header": {
        "label": "📦 سرتیتر پیام نتیجه نهایی اسکن",
        "vars": ["pattern_summary", "color_summary", "symbol_summary", "result_summary"],
        "default": (
            "🧩 مدل انتخاب‌شده: {pattern_summary}\n"
            "🎨 بک‌گراند انتخاب‌شده: {color_summary}\n"
            "🏷 نماد انتخاب‌شده: {symbol_summary}\n\n"
            "{result_summary}\n\n"
        ),
    },
    "msg_force_sub_alert": {
...
MIN_TON_PACKAGE = 0.01   # کمترین مبلغ مجاز برای هر بسته/محصول (قبلاً 1 بود)
MAX_TON_PACKAGE = 10000


def get_ton_package_options() -> tuple[float, ...]:
    raw = runtime_settings.get("ton_packages", DEFAULT_SETTINGS["ton_packages"])
    values: list[float] = []
    for part in str(raw).replace("،", ",").split(","):
        ...
        try:
            ...
            continue
        if MIN_TON_PACKAGE <= value <= MAX_TON_PACKAGE and value not in values:
            values.append(value)
    return tuple(sorted(values)) or (1, 2, 5, 10, 20, 50)
...
_KNOWN_LABEL_ALTERNATION = r"Model|Pattern|Backdrop|Background|Color|Symbol|Quantity|Rarity"
...
# نماد (Symbol) - سومین ویژگی هر گیفت، دقیقاً با همون منطق Model/Backdrop
# از توضیحات صفحه استخراج می‌شود.
RE_TEXT_SYMBOL = re.compile(
    rf"\b(?:Symbol)\b\s*(?::|\|)\s*(.+?)(?=\s+\b(?:{_KNOWN_LABEL_ALTERNATION})\b\s*(?::|\|)|\s*$)",
    re.I,
)

# -- رجکس‌های زیر قبلاً استفاده می‌شدند ولی هیچ‌جای فایل تعریف نشده بودند --
# نتیجه: هر بار normalize_base_link/extract_slug/extract_traits_layer0/
# extract_image_candidates/on_force_sub_channel_input/فایل‌نام‌سازی صدا زده
# می‌شدند، پایتون با NameError کرش می‌کرد - یعنی «شروع سرچ»، «وسط اسکن»
# (استخراج مدل/بک‌گراند از og:description) و «ارسال لینک» همه از همین یک
# ریشه خراب بودند. این‌جا دوباره تعریف شدند:

# لینک پایه مثل https://t.me/nft/CollectionName- یا t.me/nft/Name-1 ->
# گروه ۱ = اسلاگ (Name/CollectionName)، تحمل‌پذیر نسبت به "-" داخل خودِ اسلاگ
RE_NORMALIZE_BASE_LINK = re.compile(r"t\.me/nft/([A-Za-z0-9_-]+)-\d*/?(?:[?#].*)?$", re.I)

# از روی لینک پایه‌ی کامل (که همیشه با "-" تمام می‌شود) اسلاگ را استخراج می‌کند
RE_EXTRACT_SLUG = re.compile(r"/nft/([^/]+)-$", re.I)

# @channel_username یا https://t.me/channel_username -> گروه ۱ = channel_username
RE_CHANNEL_T_ME = re.compile(r"t\.me/([A-Za-z0-9_]+)", re.I)

# تگ کامل <meta ...> برای پیمایش هر متا-تگ صفحه
RE_META_TAG = re.compile(r"<meta\b[^>]*>", re.I)
# تشخیص اینکه این متا-تگ همان og:description یا twitter:description است
RE_META_IS_DESCRIPTION = re.compile(r'(?:property|name)\s*=\s*"(?:og|twitter):description"', re.I)
# استخراج مقدار attribute مربوط به content="..." از یک متا-تگ
RE_META_CONTENT_ATTR = re.compile(r'content\s*=\s*"([^"]*)"', re.I)

# آدرس عکس داخل CSS مثل background-image:url('https://...')
RE_STYLE_URL = re.compile(r"""url\(\s*['"]?([^'")]+)['"]?\s*\)""", re.I)
# fallback: هر URL مستقیم عکس داخل HTML خام (وقتی سلکتورهای بالا چیزی پیدا نکردند)
RE_IMG_URL_FALLBACK = re.compile(
    r"""https?://[^\s"'<>]+\.(?:jpg|jpeg|png|webp|gif)(?:\?[^\s"'<>]*)?""", re.I
)

# جداکننده‌ی underscore داخل اسلاگ خام (Plush_Pepe -> Plush Pepe)
RE_SLUG_UNDERSCORE = re.compile(r"_+")
# مرز camelCase داخل اسلاگ خام (PlushPepe -> Plush Pepe)
RE_SLUG_CAMELCASE = re.compile(r"(?<=[a-z0-9])(?=[A-Z])")

# هر کاراکتر غیرمجاز برای نام فایل -> با "_" جایگزین می‌شود
...
class AdminFlow(StatesGroup):
    ...
    waiting_ton_packages = State()          # منتظر مبالغ بسته‌های پرداخت TON
...
class SessionData:
    ...
    symbol_emoji: dict = field(default_factory=dict)    # نام نماد -> (emoji_id, fallback) پرمیوم اختصاصی
...
def canonical_db_base_link(base_link: str) -> str:
    """
    Canonical key for Telegram NFT collection links.
    All collection slugs are stored/queried in lowercase so different
    capitalization can never create a second cache/collection.
    """
    if not base_link:
        return base_link
    base_link = base_link.strip().replace("\u200b", "").replace("\ufeff", "")
    m = re.match(r"^https?://t\.me/nft/([^/]+)-$", base_link, re.I)
    if m:
        return f"https://t.me/nft/{m.group(1).lower()}-"
    return base_link.lower()


def _migrate_collection_links_to_lowercase():
    """
    One-time/automatic migration for old databases that may contain the same
    collection under different capitalization. Duplicates are merged before
    the old rows are removed.
    """
    conn = _get_conn()

    # table -> ordered columns, newest row wins when the same logical key
    # exists more than once.
    specs = {
        "collections": (
            ["base_link", "total_available", "updated_at"],
            ("base_link",),
            "updated_at",
        ),
        "admin_added_collections": (
            ["base_link", "total_items", "status", "added_at", "last_refreshed_at", "search_locked"],
            ("base_link",),
            "last_refreshed_at",
        ),
        "items": (
            ["base_link", "idx", "valid", "pattern", "color", "checked_at"],
            ("base_link", "idx"),
            "checked_at",
        ),
        "nft_media": (
            ["base_link", "pattern", "image_path", "colors", "border_color", "cached_at"],
            ("base_link", "pattern"),
            "cached_at",
        ),
        "nft_media_combo": (
    ...
    for table, (columns, key_columns, newest_column) in specs.items():
        try:
            rows = conn.execute(
        ...
        # Dict keeps one canonical row for every logical key.
        merged = {}
        for row in rows:
            values = list(row)
            values[0] = canonical_db_base_link(values[0])
            key = tuple(values[columns.index(c)] for c in key_columns)
            if key not in merged:
                merged[key] = values
            elif newest_column:
                ni = columns.index(newest_column)
                old = merged[key]
                if (values[ni] or 0) >= (old[ni] or 0):
                    merged[key] = values

        # Only rewrite when something actually needs canonicalization/dedup.
        if len(merged) == len(rows) and all(r[0] == canonical_db_base_link(r[0]) for r in rows):
            continue
...
def _init_db():
    ...
    # مهاجرت برای دیتابیس‌های قدیمی‌تر که هنوز ستون symbol (نماد) رو ندارند.
    # مقدار symbol برای آیتم‌های قدیمی که قبل از این نسخه اسکن شده‌اند خالی
    # می‌ماند تا وقتی که ادمین از پنل «افزودن»، دکمه‌ی «🏷 افزودن نماد» را
    # برای همون کالکشن بزند (فقط همون‌جا/همون لحظه یک اسکن مجدد واقعی انجام
    # می‌شود، نه به‌صورت خودکار برای همه‌ی کالکشن‌ها).
    try:
        _get_conn().execute("ALTER TABLE items ADD COLUMN symbol TEXT")
    ...
    # ایندکس مخصوص فیلتر/DISTINCT روی نماد (Symbol) - سومین معیار سرچ،
    # دقیقاً به همون دلیلی که idx_items_pattern_color برای مدل/بک‌گراند لازم بود.
    _get_conn().execute(
        "CREATE INDEX IF NOT EXISTS idx_items_symbol ON items (base_link, valid, symbol)"
    )
    ...
    # Canonicalize old records too, so a DB created by an older version
    # cannot keep separate DurovsCap/durovscap entries.
    _migrate_collection_links_to_lowercase()
    _get_conn().commit()
...
async def db_upsert_admin_collection(base_link: str, total_items: int, status: str):
    base_link = canonical_db_base_link(base_link)
    now = int(time.time())
...
async def db_get_admin_collection(base_link: str) -> Optional[tuple]:
    ...
    def _run():
        cur = _get_conn().execute(
            "SELECT base_link, total_items, status, added_at, last_refreshed_at, search_locked "
            "FROM admin_added_collections WHERE base_link=? COLLATE NOCASE",
            (base_link,),
        )
        return cur.fetchone()
...
async def db_set_collection_lock(base_link: str, locked: bool):
    ...
    def _run():
        _get_conn().execute(
            "UPDATE admin_added_collections SET search_locked=? WHERE base_link=? COLLATE NOCASE",
...
async def db_get_collection_lock(base_link: str) -> bool:
    ...
    def _run():
        cur = _get_conn().execute(
            "SELECT search_locked FROM admin_added_collections WHERE base_link=? COLLATE NOCASE", (base_link,)
        )
        row = cur.fetchone()
...
async def db_get_distinct_patterns_colors(base_link: str) -> tuple[set[str], set[str], set[str]]:
    base_link = canonical_db_base_link(base_link)
    """
    مدل‌ها/بک‌گراندها/نمادهای یکتای موجود (بین آیتم‌های معتبر) این کالکشن را
    مستقیم با SQL DISTINCT (روی ایندکس‌های idx_items_pattern_color و
    idx_items_symbol) برمی‌گرداند. برای کالکشنی که قفل است (از قبل کاملاً
    اسکن شده)، این جایگزین حلقه‌ی کند «کشف ویژگی‌ها» (خواندن دسته‌به‌دسته‌ی
    هزاران آیتم) می‌شود: به‌جای عبور مرحله‌به‌مرحله از ۱ تا چند هزار آیتم،
    همون لحظه و با یک کوئری ایندکس‌شده تمام مدل‌ها/رنگ‌ها/نمادهای موجود
    برمی‌گردند. توجه: اگر این کالکشن هنوز از دکمه‌ی «🏷 افزودن نماد» بازبینی
    نشده باشد، ستون symbol آیتم‌های قدیمی‌اش خالی است و مجموعه‌ی symbols
    برای آن‌ها ممکن است خالی برگردد.
    """
    def _run():
        cur = _get_conn().execute(
        ...
        cur = _get_conn().execute(
            "SELECT DISTINCT symbol FROM items WHERE base_link=? AND valid=1 AND symbol IS NOT NULL",
            (base_link,),
        )
        symbols = {row[0] for row in cur.fetchall()}
        return patterns, colors, symbols
...
async def db_get_cached_items(
...
) -> dict[int, tuple[bool, Optional[str], Optional[str], Optional[str]]]:
    base_link = canonical_db_base_link(base_link)
    """
    ...
    def _run():
        cur = _get_conn().execute(
            "SELECT idx, valid, pattern, color, symbol FROM items WHERE base_link=? AND idx BETWEEN ? AND ?",
            (base_link, lo, hi),
        )
        return {
            row[0]: (bool(row[1]), row[2], row[3], row[4])
...
async def db_store_items(base_link: str, rows: list[tuple[int, bool, Optional[str], Optional[str], Optional[str]]]):
    base_link = canonical_db_base_link(base_link)
    ...
    def _run():
        ...
        # نام Model/Backdrop/Symbol ذخیره شود. این باعث می‌شود «آپدیت» و
        # اسکن‌های عادی هر دو دقیقاً یک canonical value تولید کنند.
        clean_rows = [
            (base_link, idx, int(valid),
             _canonicalize_trait_value(pattern),
             _canonicalize_trait_value(color),
             _canonicalize_trait_value(symbol), now)
            for idx, valid, pattern, color, symbol in rows
        ]
        _get_conn().executemany(
            "INSERT OR REPLACE INTO items (base_link, idx, valid, pattern, color, symbol, checked_at) VALUES (?,?,?,?,?,?,?)",
            clean_rows,
        )
        _get_conn().commit()
...
async def db_query_filtered_items(
    ...
    آیتم‌های نامرتبط. اگر patterns/colors/symbols خالی باشند یعنی «همه» انتخاب شده.
    """
    def _run():
...
async def db_create_ton_payment(user_id: int, ton_amount: float, credit_amount: int, comment: str) -> str:
                ...
                for order_id, user_id, ton_amount, credit_amount, comment in pending:
                    for tx in by_comment.get(comment, []):
                        ...
                        try:
                            ...
                            continue
                        if value != round(ton_amount * 1_000_000_000):
                            continue
...
def _process_fetched_batch(
...
) -> tuple[dict[int, tuple[str, bool, Optional[str], Optional[str], Optional[str], bool]], list[tuple[int, bool, Optional[str], Optional[str], Optional[str]]]]:
    """
    ...
    """
    results: dict[int, tuple[str, bool, Optional[str], Optional[str], Optional[str], bool]] = {}
    new_rows: list[tuple[int, bool, Optional[str], Optional[str], Optional[str]]] = []
    for i, url, html, net_err in fetched_items:
        if net_err:
            results[i] = (url, False, None, None, None, True)
            continue
        valid = is_page_valid(html)
        pattern = color = symbol = None
        if valid:
            traits = extract_traits(html)
            pattern = traits.get("pattern")
            color = traits.get("color")
            symbol = traits.get("symbol")
        results[i] = (url, valid, pattern, color, symbol, False)
        new_rows.append((i, valid, pattern, color, symbol))
    return results, new_rows


async def scan_indices_cached(
    base_link: str, indices: list[int]
) -> list[tuple[str, bool, Optional[str], Optional[str], Optional[str], bool]]:
    base_link = canonical_db_base_link(base_link)
    """
    ...
    خروجی به همون ترتیب indices ورودی: (url, valid, pattern, color, symbol, net_err)
    """
    cached = await db_get_cached_items(base_link, indices)
    results: dict[int, tuple[str, bool, Optional[str], Optional[str], Optional[str], bool]] = {}
    to_fetch = [i for i in indices if i not in cached]

    for i, (valid, pattern, color, symbol) in cached.items():
        results[i] = (f"{base_link}{i}", valid, pattern, color, symbol, False)
...
def _layer_zero_meta_description(html: str) -> Optional[dict]:
    ...
    (این تابع حالا هر سه‌ی Model/Backdrop/Symbol را با همین منطق استخراج می‌کند.)
    ...
    for line in re.split(r"[\r\n]+", content):
        ...
        # "Quantity"/"Rarity"/... همچنان عمداً نادیده گرفته می‌شوند
...
def _layer_two_text(html: str) -> Optional[dict]:
    ...
    if not result:
        ...
        m_symbol = RE_TEXT_SYMBOL.search(text_for_fallback)
        if m_pattern:
        ...
        if m_color:
            ...
            if cleaned:
                result["color"] = cleaned
        if m_symbol:
            cleaned = _clean_trait_value(m_symbol.group(1))
            if cleaned:
                result["symbol"] = cleaned
...
def extract_slug(base_link: str) -> str:
    ...
    return m.group(1).lower() if m else canonical_db_base_link(base_link)
...
async def _window_last_valid(base_link: str, start_idx: int, window: int) -> tuple[Optional[int], bool]:
    ...
    for attempt in range(max_retries):
        ...
        last_valid = None
        error_count = 0
        for i, (url, valid, pattern, color, symbol, net_err) in zip(idxs, results):
            if net_err:
                error_count += 1
                continue
            if valid:
...
def build_toggle_marks_keyboard() -> InlineKeyboardMarkup:
    ...
    kb.button(text="🔙 بازگشت به دکمه‌ها", callback_data="admin:button_settings")
    kb.adjust(1)
    return kb.as_markup()
...
async def admin_button_text_start(call: CallbackQuery,state:FSMContext):
    ...
    key=call.data.split(":",2)[2]; await call.answer(); await state.update_data(button_key=key); await state.set_state(AdminFlow.waiting_button_text); await safe_edit(call.message,"✏️ متن جدید دکمه را بفرست:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin:button_settings")]]))
...
async def admin_button_emoji_start(call:CallbackQuery,state:FSMContext):
    if not is_admin(call.from_user.id): return
    key=call.data.split(":",2)[2]; await call.answer(); await state.update_data(button_key=key); await state.set_state(AdminFlow.waiting_button_emoji); await safe_edit(call.message,"✨ فقط یک ایموجی پرمیوم بفرست:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin:button_settings")]]))
...
async def admin_msgs_menu(call: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await call.answer()
    await safe_edit(call.message, _msgs_menu_text(), reply_markup=build_msgs_menu_keyboard())
...
async def admin_msg_set_text(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        ...
        "عددی‌اش نیست." + hint,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin:message_settings")]]),
    )
...
async def admin_trait_emoji_menu(call: CallbackQuery, state: FSMContext):
    ...
    kb.button(text="🏷 ایموجی نمادها (Symbol)", callback_data=f"traitemoji_list:symbol:{slug}:0")
...
async def _render_trait_emoji_list_view(call: CallbackQuery, trait_type: str, slug: str, page: int):
    ...
    patterns, colors, symbols = await db_get_distinct_patterns_colors(base_link)
    values_by_type = {"pattern": patterns, "color": colors, "symbol": symbols}
    values = sorted(values_by_type.get(trait_type, set()))
    ...
    label = {"pattern": "مدل‌ (Pattern)", "color": "بک‌گراند (Backdrop)", "symbol": "نماد (Symbol)"}.get(trait_type, trait_type)
    await safe_edit(call.message, 
...
async def admin_trait_emoji_pick(call: CallbackQuery, state: FSMContext):
    ...
    base_link = f"https://t.me/nft/{slug}-"
    patterns, colors, symbols = await db_get_distinct_patterns_colors(base_link)
    values = sorted({"pattern": patterns, "color": colors, "symbol": symbols}.get(trait_type, set()))
    try:
...
async def admin_on_add_link(message: Message, state: FSMContext):
    ...
    if base:
        base = canonical_db_base_link(base)
    if not base:
        await message.answer("❌ فرمت لینک درست نیست. دوباره امتحان کن (مثال: https://t.me/nft/Name-)")
        return
    await state.clear()

    # اگر این کالکشن (صرف‌نظر از بزرگی/کوچکی حروف لینک - چون base همیشه
    # canonical/lowercase است) قبلاً کامل اضافه شده، دوباره از صفر
    # اسکن/اضافه نکن؛ فقط وضعیت فعلی‌اش را نشان بده. این دقیقاً همون چیزیه
    # که باعث می‌شد فرستادن دوباره‌ی همون لینک با حروف متفاوت، انگار
    # کالکشن جدیدیه، از اول اسکن و اضافه بشه.
    existing = await db_get_admin_collection(base)
    if existing and base not in _admin_scan_running:
        _, _, existing_status, _, _, _ = existing
        if existing_status == "done":
            slug = extract_slug(base)
            await message.answer(
                "ℹ️ این کالکشن قبلاً به دیتابیس اضافه شده (حتی اگر حروف بزرگ/کوچک "
                "لینکی که فرستادی فرق داشت، چون کلید ذخیره‌سازی به حروف بزرگ/کوچک "
                "حساس نیست، همون کالکشن قبلی شناسایی شد).\n"
                f"🔗 <code>{base}</code>\n\n"
                "برای بروزرسانی یا مدیریتش از دکمه پایین استفاده کن؛ اگر واقعاً "
                "می‌خوای از صفر دوباره اسکن بشه، اول از «🗑 حذف کامل این کالکشن» "
                "پاکش کن و دوباره همین لینک را بفرست.",
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[[InlineKeyboardButton(text="📦 مشاهده/مدیریت", callback_data=f"addview:{slug}")]]
                ),
            )
            return
...
async def _render_admin_add_view(call: CallbackQuery, slug: str):
    ...
    kb.button(
        text=("⏳ در حال افزودن نماد..." if running else "🏷 افزودن نماد (Symbol) به این کالکشن"),
        callback_data="noop" if running else f"addsymbols:{slug}",
    )
    kb.button(
    ...
    kb.button(text="✨ ایموجی پرمیوم مدل/بک‌گراند/نماد", callback_data=f"traitemoji_menu:{slug}")
    ...
    await safe_edit(call.message, 
        ...
        f"🏷 «افزودن نماد (Symbol)»: چون این ویژگی جدید است، کالکشن‌هایی که "
        f"قبلاً اسکن شده‌اند مقدار نماد ندارند - این دکمه *فقط با کلیک خودت* "
        f"(نه خودکار برای همه‌ی کالکشن‌ها) تمام آیتم‌های همین کالکشن را دوباره "
        f"از سایت می‌گیرد و نماد هرکدام را هم استخراج و ذخیره می‌کند تا در "
        f"سرچ کاربرها به‌عنوان معیار سوم (در کنار مدل و بک‌گراند) قابل انتخاب "
        f"باشد. مدل/بک‌گراند هم همزمان دوباره تأیید می‌شوند.\n"
...
async def admin_add_toggle_lock(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        await call.answer("⛔ دسترسی نداری.", show_alert=True)
        return
    slug = call.data.split(":", 1)[1].strip().lower()
    base_link = canonical_db_base_link(f"https://t.me/nft/{slug}-")
...
@router.callback_query(F.data.startswith("addsymbols:"))
async def admin_add_symbols(call: CallbackQuery, state: FSMContext):
    """
    دکمه‌ی اختصاصی «🏷 افزودن نماد»: زیر کاپوت دقیقاً همون منطق
    admin_rediscover_traits را صدا می‌زند (چون یک اسکن مجدد کامل، مدل/
    بک‌گراند/نماد را همزمان و از همون صفحه استخراج می‌کند) - تنها فرقش
    پیام وضعیت است تا برای ادمین مشخص باشد که این عملیات مخصوص تکمیل
    نمادهای این کالکشن است. هیچ کالکشن دیگری لمس نمی‌شود و هیچ‌جا خودکار
    صدا زده نمی‌شود - فقط با همین کلیک.
    """
    if not is_admin(call.from_user.id):
        await call.answer("⛔ دسترسی نداری.", show_alert=True)
        return
    ...
    if base_link in _admin_scan_running:
        await call.answer("⚠️ این کالکشن همین الان هم در حال اسکن است.", show_alert=True)
        return
    await call.answer("🏷 افزودن نماد شروع شد...")
    status_msg = await call.message.answer(
        f"⏳ شروع افزودن نماد (Symbol) به کالکشن...\n<code>{base_link}</code>"
    )
    asyncio.create_task(admin_rediscover_traits(base_link, status_msg))
...
async def admin_recheck_gaps(base_link: str, status_msg: Message):
    ...
    try:
        ...
        for start in range(0, len(gap_idxs), batch_size):
            ...
            for (url, valid, pattern, color, symbol, net_err) in results:
                if valid:
                    newly_valid += 1
            checked += len(batch)
...
async def on_base_link(message: Message, state: FSMContext):
    ...
    if collection_locked and cached_total and cached_total > 0:
        ...
        patterns, colors, symbols = await db_get_distinct_patterns_colors(base)
        sess.available_patterns = patterns
        sess.available_colors = colors
        sess.available_symbols = symbols
        ...
        while idx <= discovery_cap and stable_streak < DISCOVERY_STABILITY_WINDOW:
            ...
            for url, valid, pattern, color, symbol, net_err in results:
                ...
                if color and color not in sess.available_colors:
                    ...
                    is_new = True
                if symbol and symbol not in sess.available_symbols:
                    sess.available_symbols.add(symbol)
                    is_new = True
                if is_new:
...
async def _go_to_menus(user_id: int, send_func, state: FSMContext):
    ...
    sess.symbol_emoji = await db_get_trait_emoji_map(sess.base_link, "symbol")
...
async def on_pattern_toggle(call: CallbackQuery, state: FSMContext):
    ...
    if value == "__all__":
        if sess.select_all_colors or sess.select_all_symbols:
            await call.answer(
...
async def on_color_toggle(call: CallbackQuery, state: FSMContext):
    ...
    if value == "__confirm__":
        if not sess.selected_colors:
            ...
            return
        if sess.available_symbols:
            await call.answer()
            await _edit_text_with_toggle_kb(
                call.message,
                get_msg("msg_symbol_prompt", trait_list=""),
                sess.available_symbols, sess.selected_symbols, "sym",
                select_all_active=sess.select_all_symbols,
                select_all_disabled=sess.select_all_patterns or sess.select_all_colors,
                emoji_map=sess.symbol_emoji,
            )
            await state.set_state(ScanFlow.waiting_symbol_choice)
            return
        await call.answer()
    ...
    if value == "__all__":
        if sess.select_all_patterns or sess.select_all_symbols:
            await call.answer(
...
@router.callback_query(F.data.startswith("sym:"), StateFilter(ScanFlow.waiting_symbol_choice))
async def on_symbol_toggle(call: CallbackQuery, state: FSMContext):
    """
    گام سوم انتخاب (نماد/Symbol) - دقیقاً مثل انتخاب بک‌گراند، فقط به‌جای
    رفتن به گام بعدی، مستقیم اسکن نهایی را شروع می‌کند.
    """
    sess = sessions[call.from_user.id]
    value = _cb_decode(call.data.split(":", 1)[1])

    if value == "__confirm__":
        if not sess.selected_symbols:
            await call.answer(get_msg("msg_symbol_confirm_min_one"), show_alert=True)
            return
        await call.answer()
        await safe_edit(call.message, get_msg("msg_scan_starting"))
        await run_scan(call.message, call.from_user.id, state)
    ...
    if value == "__all__":
        if sess.select_all_patterns or sess.select_all_colors:
            await call.answer(
                get_msg("msg_symbol_all_disabled_by_others"),
                show_alert=True,
            )
            return
        if sess.select_all_symbols:
            sess.select_all_symbols = False
            sess.selected_symbols.clear()
        else:
            sess.select_all_symbols = True
            sess.selected_symbols = set(sess.available_symbols)
        await _edit_markup_with_toggle_kb(
            call.message,
            sess.available_symbols, sess.selected_symbols, "sym",
            select_all_active=sess.select_all_symbols,
    ...
    if value in sess.selected_symbols:
        sess.selected_symbols.discard(value)
    else:
        sess.selected_symbols.add(value)
    sess.select_all_symbols = False  # انتخاب دستی -> حالت "همه" دیگر فعال نیست
...
async def run_scan(message: Message, user_id: int, state: FSMContext):
    ...
    if await db_check_full_coverage(sess.base_link, sess.max_items):
        ...
        symbols = None if sess.select_all_symbols else (sess.selected_symbols or None)
        matched_idxs = await db_query_filtered_items(sess.base_link, patterns, colors, sess.max_items, symbols)
        ...
        def process_result(url: str, valid: bool, pattern: Optional[str], color: Optional[str], symbol: Optional[str], net_err: bool):
            nonlocal scanned
            if net_err:
                return
            scanned += 1
            ...
            # فیلتر متقاطع دقیق: پترن، رنگ و نماد چک می‌شوند (نه همه ویژگی‌ها هم‌زمان)
            ...
            symbol_ok = (not sess.selected_symbols) or (symbol in sess.selected_symbols)
            if pattern_ok and color_ok and symbol_ok:
        ...
        for start in range(0, len(all_idxs), batch_size):
            ...
            for i, (url, valid, pattern, color, symbol, net_err) in zip(batch, results):
                if net_err:
                    failed_idxs.append(i)
                    continue
                process_result(url, valid, pattern, color, symbol, net_err)
                if stop_now:
                    break
        ...
        if failed_idxs and not stop_now:
            ...
            for url, valid, pattern, color, symbol, net_err in retry_results:
                if net_err:
    ...
    if sess.select_all_symbols:
        symbol_summary = get_msg("msg_all_symbols_label")
    elif sess.selected_symbols:
        symbol_summary = "، ".join(sorted(sess.selected_symbols))
    else:
        symbol_summary = "—"
...
def build_charge_packages_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for ton in get_ton_package_options():
        kb.button(text=f"💎 {ton:g} TON  →  🤖 {credits_for_ton(ton)} اعتبار", callback_data=f"charge:pkg:{ton:g}")
    kb.adjust(2)
    kb.row(InlineKeyboardButton(text="🔙 منوی اصلی", callback_data="menu:home"))
    return kb.as_markup()


def build_charge_selected_kb(ton: float) -> InlineKeyboardMarkup:
    credits = credits_for_ton(ton)
    kb = InlineKeyboardBuilder()
    # دو ردیف نمایشیِ متصل: عدد TON و اعتبار دقیقاً از یک مقدار محاسبه می‌شوند.
    kb.row(InlineKeyboardButton(text=f"🔹 تعداد TON: {ton:g}", callback_data="menu:noop"))
    kb.row(InlineKeyboardButton(text=f"🤖 اعتبار استفاده از ربات: {credits}", callback_data="menu:noop"))
    kb.row(InlineKeyboardButton(text=runtime_settings.get("charge_auto_button", DEFAULT_SETTINGS["charge_auto_button"]), callback_data=f"charge:auto:{ton:g}", icon_custom_emoji_id=runtime_settings.get("charge_auto_button_id") or None))
    kb.row(InlineKeyboardButton(text=runtime_settings.get("charge_manual_button", DEFAULT_SETTINGS["charge_manual_button"]), callback_data=f"charge:manual:{ton:g}", icon_custom_emoji_id=runtime_settings.get("charge_manual_button_id") or None))
...
async def charge_pick_package(call: CallbackQuery, state: FSMContext):
    try:
        ton = float(call.data.split(":")[-1])
    except ValueError:
        await call.answer("مقدار نامعتبر است.", show_alert=True)
        return
    if ton not in get_ton_package_options():
        await call.answer("مقدار نامعتبر است.", show_alert=True)
        return
    await state.update_data(charge_ton=ton)
    await call.answer()
    await call.message.edit_text(get_msg("msg_charge_details", ton=f"{ton:g}", credits=credits_for_ton(ton)), reply_markup=build_charge_selected_kb(ton))
...
async def charge_auto_start(call: CallbackQuery, state: FSMContext):
    ...
    amount_nano = round(ton * 1_000_000_000)
    ...
    await state.clear()
    await call.answer()
    await call.message.answer(get_msg("msg_charge_auto", ton=f"{ton:g}", credits=credits, comment=comment), reply_markup=kb.as_markup())
...
async def charge_manual_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"🧾 <b>پرداخت دستی</b>\n\n"
        f"💎 مبلغ: <b>{ton:g} TON</b>\n"
        f"🤖 اعتبار موردنظر: <b>{credits}</b>\n\n"
        f"به ولت زیر <b>دقیقاً {ton:g} TON</b> بفرست و سپس عکس/فایل رسید را همین‌جا ارسال کن:\n\n"
...
async def charge_manual_receipt(message: Message, state: FSMContext):
    data = await state.get_data()
    ton = float(data.get("charge_ton") or 0)
    ...
    caption = (
        ...
        f"💎 مبلغ اعلام‌شده: <b>{ton:g} TON</b>\n"
        f"🤖 اعتبار موردنظر: <b>{credits}</b>\n\n"
...
def build_pricing_keyboard() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.button(text="✏️ تغییر قیمت هر اعتبار", callback_data="pricing:set")
    kb.button(text="💎 ویرایش مقدار TON پرداختی", callback_data="pricing:packages")
    kb.button(text="👤 شارژ موجودی کاربر", callback_data="admin:credit")
    kb.button(text="🔙 بازگشت به پنل", callback_data="admin:back")
    kb.adjust(1)
    return kb.as_markup()


def _pricing_menu_text() -> str:
    p = get_credit_price_ton()
    packages = get_ton_package_options()
    return (
        "💰 <b>قیمت اعتبار و شارژ</b>\n\n"
        f"هر ۱ اعتبار = <b>{p:g} TON</b>\n"
        f"بسته‌های فعلی پرداخت: <b>{'، '.join(f'{x} TON' for x in packages)}</b>\n\n"
        "از همین بخش می‌توانی قیمت اعتبار، مبالغ TON پرداختی و شارژ دستی موجودی کاربران را مدیریت کنی."
    )
...
async def admin_pricing(call: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await call.answer()
    await safe_edit(call.message, _pricing_menu_text(), reply_markup=build_pricing_keyboard())
...
async def admin_pricing_set(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        ...
        return
    await call.answer()
    await state.set_state(AdminFlow.waiting_credit_price)
    await safe_edit(call.message, "✏️ قیمت هر اعتبار را به TON بفرست. مثال: <code>0.2</code>", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin:pricing")]]))
...
async def admin_pricing_input(message: Message, state: FSMContext):
    ...
    if value <= 0 or value > 100:
        await message.answer("❌ قیمت باید بین 0 و 100 TON باشد.")
        return
    await db_set_setting("credit_price_ton", str(value))
    await state.clear()
    await message.answer(f"✅ قیمت هر اعتبار: <b>{value:g} TON</b>", reply_markup=build_pricing_keyboard())


@router.callback_query(F.data == "pricing:packages")
async def admin_packages_start(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        await call.answer("⛔", show_alert=True)
        return
    await call.answer()
    current = ", ".join(map(str, get_ton_package_options()))
    await state.set_state(AdminFlow.waiting_ton_packages)
    await safe_edit(call.message,
        "💎 <b>مبالغ TON پرداختی</b>\n\n"
        f"مقادیر فعلی: <code>{current}</code>\n\n"
        "مقادیر جدید را جداشده با کاما بفرست (اعشار/ممیز هم مجاز است).\n"
        "مثال: <code>0.01,0.5,1,2.5,5,10,25,50</code>\n\n"
        f"حداکثر ۱۰ مقدار و هر مقدار بین {MIN_TON_PACKAGE:g} تا {MAX_TON_PACKAGE:g} TON.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin:pricing")]]),
    )


@router.message(StateFilter(AdminFlow.waiting_ton_packages))
async def admin_packages_input(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    ...
    try:
        for part in raw.split(","):
            ...
            if not part:
                continue
            value = float(part)
            if not MIN_TON_PACKAGE <= value <= MAX_TON_PACKAGE:
                raise ValueError
            if value not in values:
                values.append(value)
    except ValueError:
        await message.answer(
            f"❌ فقط عدد (اعشار هم مجاز است) بین {MIN_TON_PACKAGE:g} تا {MAX_TON_PACKAGE:g} بفرست؛ "
            f"مثال: <code>0.01,0.5,1,2.5,10,20,50</code>"
        )
        return
    if not values or len(values) > 10:
        await message.answer("❌ باید بین 1 تا 10 مبلغ وارد کنی.")
        return
    values.sort()
    await db_set_setting("ton_packages", ",".join(f"{v:g}" for v in values))
    await state.clear()
    await message.answer("✅ مبالغ TON پرداختی ذخیره شد: " + ", ".join(f"{v:g} TON" for v in values), reply_markup=build_pricing_keyboard())
...
async def admin_credit_amount(message: Message, state: FSMContext):
    ...
    if not uid:
        await state.clear()
        await message.answer("❌ کاربر مشخص نشده است.", reply_markup=build_pricing_keyboard())
        return
    await state.clear()
    ...
    await message.answer(
        ...
        reply_markup=build_pricing_keyboard(),
    )
    try:
