...
# Patterns used by the existing parsing helpers below.
...
RE_META_CONTENT_ATTR = re.compile(
    r'content\s*=\s*["\'](.*?)["\']',
    re.I | re.S,
)
RE_STYLE_URL = re.compile(
    r'url\(\s*[\'"]?([^\'")]+)[\'"]?\s*\)',
    re.I,
)
...
RE_EXTRACT_SLUG = re.compile(
    r'(?:https?://)?(?:t\.me|telegram\.me)/nft/([^/?#-]+)-?(?:\d+)?/?(?:[?#].*)?$',
    re.I,
)
RE_NORMALIZE_BASE_LINK = re.compile(
    r'(?:https?://)?(?:t\.me|telegram\.me)/nft/([^/?#-]+)(?:-\d+)?/?(?:[?#].*)?$',
    re.I,
)
RE_CHANNEL_T_ME = re.compile(
    r'(?:https?://)?t\.me/([A-Za-z0-9_]+)',
    re.I,
)
RE_SAFE_FILENAME = re.compile(r'[^\w.-]+', re.UNICODE)
