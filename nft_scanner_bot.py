...
class SQLiteFSMStorage(BaseStorage):
    """Persistent aiogram FSM storage backed by the Railway SQLite volume.

    The FSM uses its own SQLite connection and lock.  This is deliberately
    separate from the scanner's thread-local database connections: FSM reads
    and writes are tiny and must never depend on a transient worker-thread
    connection or on a concurrent PRAGMA/journal-mode initialization.
    """
    ...
    def _key_id(key: StorageKey) -> str:
        # Keep every StorageKey dimension so users/topics/business contexts
        # can never accidentally share FSM state.
        values = (
    ...
    def _execute_sync(self, sql: str, params=(), *, fetchone=False, fetchall=False):
    ...
    async def set_state(self, key: StorageKey, state=None) -> None:
        ...
        async with self._lock:
            await asyncio.to_thread(
                ...
                """,
                (key_id, state_value, key_id, int(time.time())),
            )
    ...
    async def get_state(self, key: StorageKey) -> Optional[str]:
        ...
        async with self._lock:
            row = await asyncio.to_thread(
                self._execute_sync,
                "SELECT state FROM fsm_storage WHERE key_id=?",
                (key_id,),
                fetchone=True,
            )
    ...
    async def set_data(self, key: StorageKey, data) -> None:
        if not isinstance(data, dict):
            raise TypeError(f"FSM data must be a dict, got {type(data).__name__}")
        key_id = self._key_id(key)
        ...
        async with self._lock:
            await asyncio.to_thread(
                ...
                """
                INSERT INTO fsm_storage (key_id, state, data, updated_at)
                VALUES (?, (SELECT state FROM fsm_storage WHERE key_id=?), ?, ?)
                ON CONFLICT(key_id) DO UPDATE SET
                    data=excluded.data,
                    updated_at=excluded.updated_at
                """,
    ...
    async def get_data(self, key: StorageKey) -> dict:
        ...
        async with self._lock:
            row = await asyncio.to_thread(
                self._execute_sync,
                "SELECT data FROM fsm_storage WHERE key_id=?",
                (key_id,),
                fetchone=True,
            )
            ...
            def _update():
                ...
                if row and row[0]:
                    try:
                        ...
                        log.exception("Invalid FSM data in SQLite for key %s", key_id)
                current.update(data)
                encoded = json.dumps(current, ensure_ascii=False, separators=(",", ":"), default=str)
                self._conn.execute(
                    """
                    INSERT INTO fsm_storage (key_id, state, data, updated_at)
                    VALUES (?, (SELECT state FROM fsm_storage WHERE key_id=?), ?, ?)
    ...
    async def get_value(self, key: StorageKey, dict_key: str, default=None):
