...
RE_PERCENT_SUFFIX = re.compile(r"\s*\d+(\.\d+)?%\s*$")
...
RE_TEXT_PATTERN = re.compile(
    rf"\b(?:Model|Pattern)\b\s*:?\s*([A-Za-z0-9 _\-]+?)(?=\s*\b(?:{_KNOWN_LABEL_ALTERNATION})\b|\s*$)",
    re.I,
)
RE_TEXT_COLOR = re.compile(
    rf"\b(?:Backdrop|Background|Color)\b\s*:?\s*([A-Za-z0-9 _\-]+?)(?=\s*\b(?:{_KNOWN_LABEL_ALTERNATION})\b|\s*$)",
    re.I,
)
# -- لایه ۰ (جدید): استخراج از متا-تگ توضیحات صفحه -------------------------
# تلگرام برای هر آیتم NFT همیشه یک <meta property="og:description" .../>
# و/یا <meta name="twitter:description" .../> تولید می‌کند که متن *تمیز*
# «Model: X\nBackdrop: Y\nSymbol: Z» را بدون هیچ درصد رریتی/تگ HTML اضافه
# داخلش دارد؛ برخلاف جدول HTML که مقدار و درصد رریتی گاهی بدون فاصله به‌هم
# می‌چسبند (باعث تولید مقدار زباله/اشتباه می‌شد)، منبع دقیق و پایدار همینه.
RE_META_TAG = re.compile(r"<meta\s+[^>]*>", re.I)
RE_META_IS_DESCRIPTION = re.compile(
    r'(?:property|name)\s*=\s*["\'](?:og:description|twitter:description)["\']',
    re.I,
)
RE_META_CONTENT_ATTR = re.compile(r'content\s*=\s*["\']([^"\']*)["\']', re.I | re.S)
RE_STYLE_URL = re.compile(r"url\(['\"]?(.*?)['\"]?\)")
RE_IMG_URL_FALLBACK = re.compile(r"https?://[^\s\"'<>]+\.(?:jpg|jpeg|png|webp)", re.I)
RE_SLUG_UNDERSCORE = re.compile(r"[_\-]+")
RE_SLUG_CAMELCASE = re.compile(r"(?<=[a-z0-9])(?=[A-Z])")
RE_EXTRACT_SLUG = re.compile(r"nft/([A-Za-z0-9_]+)-$")
RE_NORMALIZE_BASE_LINK = re.compile(r"t\.me/nft/([A-Za-z0-9_]+?)-?\d*$")
RE_CHANNEL_T_ME = re.compile(r"t\.me/([A-Za-z0-9_]+)")
...
async def db_store_items(base_link: str, rows: list[tuple[int, bool, Optional[str], Optional[str]]]):
    ...
    def _run():
        _get_conn().executemany(
            ...
            [(base_link, idx, int(valid), pattern, color, now) for idx, valid, pattern, color in rows],
        )
        _get_conn().commit()
...
async def db_try_consume_search(user_id: int) -> tuple[bool, str]:
    ...
    return False, (
        ...
        f"هر ۱ GRAM = {SEARCHES_PER_GRAM} سرچ."
    )
...
async def find_web_related_models(backdrop: str, topic: str) -> tuple[list[tuple[str, float]], list[dict[str, str]]]:
    """
    مدل‌های مربوط به موضوع را با سرچ وب پیدا می‌کند، سپس فقط همان مدل‌هایی را
    که در دیتابیس با بک‌گراند انتخابی وجود دارند نگه می‌دارد.
    """
    models = await db_get_patterns_by_backdrop(backdrop)
    if not models:
        return [], []

    # دو پرس‌وجو: عبارت دقیق + جستجوی معنایی DDG با ~. این کار بهتر از یک
    # query ثابت برای موضوعاتی مثل Harry Potter / magic / wizard عمل می‌کند.
    queries = [
        f'"{topic}" NFT wizard magic',
        f'~"{topic}" NFT magic wizard fantasy',
    ]
...
def _layer_two_text(html: str) -> Optional[dict]:
    ...
    if not result:
        # fallback نهایی: هر متنی که شبیه "Label: Value" باشد
...
def _clean_trait_value(raw: Optional[str]) -> Optional[str]:
    """
    مقدار خام استخراج‌شده برای مدل/بک‌گراند را قبل از ذخیره پاکسازی و
    اعتبارسنجی می‌کند. این تابع دقیقاً برای رفع مشکل «حروف چرت‌وپرت/تک‌حرفی
    (مثلاً یک 'V' تنها) یا فضای اضافه‌ی اشتباهی وسط اسم» اضافه شده:

    ۱) کاراکترهای نامرئی رایج (space غیرشکن \xa0، zero-width \u200c) را
       به فاصله‌ی معمولی تبدیل و فاصله‌های تکراری را یکی می‌کند.
    ۲) اگر به‌هرحال متن تا وسط برچسب بعدی (Backdrop/Quantity/...) ادامه
       پیدا کرده باشد، از همان نقطه قطع می‌شود (دفاع دوم، مستقل از رجکس).
    ۳) درصد نایابی انتهایی (مثلاً "12.5%") حذف می‌شود.
    ۴) اگر بعد از تمام این پاکسازی‌ها چیزی به‌جز یک کاراکتر تکی (یا خالی)
       باقی نماند، None برمی‌گرداند - یعنی «تشخیص داده نشد»، نه یک مقدار
       زباله‌ی تک‌حرفی که قبلاً به اشتباه به‌عنوان مدل/رنگ ذخیره می‌شد.
    """
    if not raw:
        return None
    value = raw.replace("\xa0", " ").replace("\u200c", " ")
    value = re.sub(r"\s+", " ", value).strip(" :\u200c-")
    m = _KNOWN_LABEL_RE.search(value)
    if m and m.start() > 0:
        value = value[: m.start()].strip()
    value = RE_PERCENT_SUFFIX.sub("", value).strip()
    if len(value) < 2:
        return None
    return value
...
def build_admin_keyboard() -> InlineKeyboardMarkup:
    ...
    kb.button(text="📝 مدیریت پیام‌ها و بنرها", callback_data="admin:msgs")
...
def _msgs_menu_text() -> str:
    return (
        "📝 <b>مدیریت پیام‌ها و بنرها</b>\n\n"
        "یکی از پیام‌های زیر رو انتخاب کن تا متنش رو ببینی/تغییرش بدی.\n"
        "برای گذاشتن ایموجی پرمیوم داخل یک پیام: وقتی ازت متن جدید خواسته "
        "شد، همون پیام رو با ایموجی پرمیوم موردنظرت (از کیبورد ایموجی "
        "پرمیوم تلگرام) بفرست؛ ربات خودش کد ایموجی رو تشخیص می‌ده و ذخیره "
        "می‌کنه - لازم نیست خودت عدد ایموجی رو بدونی."
    )
...
async def on_msg_text_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        pass
    await message.answer("بازگشت به لیست پیام‌ها:", reply_markup=build_msgs_menu_keyboard())
...
async def admin_rediscover_traits(base_link: str, status_msg: Message):
    ...
    try:
        ...
        try:
            await status_msg.edit_text(
                f"✅ اسکن مجدد مدل/بک‌گراند تمام شد.\n"
                f"📦 {total_items} آیتم دوباره و از صفر بررسی و دسته‌بندی شد.\n"
                f"🔗 <code>{base_link}</code>"
            )
        except Exception:
            pass
...
async def send_main_menu(send_func, user_id: int, state: FSMContext):
    ...
    text = (
        "🎛 <b>منوی اصلی</b>\n\n"
        f"🔎 سرچ رایگان امروز: <b>{free_rem}</b> / {DAILY_FREE_FILTER_LIMIT}\n"
        f"💳 موجودی سرچ: <b>{bal}</b>\n"
        f"(هر ۱ GRAM = {SEARCHES_PER_GRAM} سرچ)\n\n"
...
async def cmd_profile(message: Message):
    ...
    text += f"(هر ۱ GRAM = {SEARCHES_PER_GRAM} سرچ)\n\n"
...
async def run_scan(message: Message, user_id: int, state: FSMContext):
    ...
    try:
        ...
        await send_main_menu(message.answer, user_id, state)
...
async def menu_profile(call: CallbackQuery, state: FSMContext):
    ...
    text += f"💳 موجودی سرچ پولی: <b>{bal}</b>\n(هر ۱ GRAM = {SEARCHES_PER_GRAM} سرچ)\n\n"
    if remaining <= 0 and bal <= 0:
        text += get_msg("msg_profile_exceeded", limit=DAILY_FREE_FILTER_LIMIT, support_username=SUPPORT_USERNAME)
    else:
        text += get_msg("msg_profile_remaining", support_username=SUPPORT_USERNAME)
...
async def menu_collections(call: CallbackQuery, state: FSMContext):
    await call.answer()
    backdrops = await db_get_all_backdrops()
    if not backdrops:
        await call.message.answer(
            "⚠️ هنوز هیچ بک‌گراندی در دیتابیس نیست.\n"
            "اول با «🔎 پیدا کردن NFT» چند کالکشن را اسکن کن.",
            reply_markup=build_main_menu_kb(),
        )
        return
    await state.set_state(CollectionFlow.waiting_backdrop)
    await state.update_data(col_backdrops=backdrops, col_backdrop=None)
    await call.message.answer(
        "🎨 <b>کالکشن‌ها</b>\n\n"
        "اول یک <b>Backdrop</b> از لیست زیر انتخاب کن "
        f"(مجموع: {len(backdrops)} رنگ در دیتابیس):",
        reply_markup=build_backdrops_kb(backdrops, 0),
    )


@router.callback_query(F.data.startswith("col:bdp:"), StateFilter(CollectionFlow.waiting_backdrop))
async def col_backdrop_page(call: CallbackQuery, state: FSMContext):
    page = int(call.data.split(":")[-1])
    data = await state.get_data()
    backdrops = data.get("col_backdrops") or await db_get_all_backdrops()
    await call.message.edit_reply_markup(reply_markup=build_backdrops_kb(backdrops, page))
    await call.answer()


@router.callback_query(F.data.startswith("col:bd:"), StateFilter(CollectionFlow.waiting_backdrop))
async def col_backdrop_pick(call: CallbackQuery, state: FSMContext):
    idx = int(call.data.split(":")[-1])
    data = await state.get_data()
    backdrops = data.get("col_backdrops") or []
    if idx < 0 or idx >= len(backdrops):
        await call.answer("نامعتبر", show_alert=True)
        return
    bd = backdrops[idx]
    await state.update_data(col_backdrop=bd)
    await state.set_state(CollectionFlow.waiting_keyword)
    await call.answer()
    await call.message.answer(
        f"✅ بک‌گراند: <b>{html_entities.escape(bd)}</b>\n\n"
        "حالا موضوع / کلمه را بفرست (مثلاً <code>Harry Potter</code> یا <code>Potion</code>).\n"
        "ربات در <b>کل دیتابیس</b> مدل‌هایی را پیدا می‌کند که نامشان شبیه این کلمه باشد "
        "و همان بک‌گراند را داشته باشند."
    )


@router.message(StateFilter(CollectionFlow.waiting_keyword))
async def col_keyword(message: Message, state: FSMContext):
    user_id = message.from_user.id
    kw = (message.text or "").strip()
    if len(kw) < 2:
        await message.answer("❌ موضوع حداقل ۲ حرف باشد.")
        return

    data = await state.get_data()
    bd = data.get("col_backdrop")
    if not bd:
        await message.answer("اول بک‌گراند را انتخاب کن.", reply_markup=build_main_menu_kb())
        await state.clear()
        return
    ...
    status = await message.answer(
        f"🌐 در حال جستجوی موضوع <b>{html_entities.escape(kw)}</b> روی وب...\n"
        f"🎨 سپس مدل‌های پیدا‌شده را با بک‌گراند <b>{html_entities.escape(bd)}</b> تطبیق می‌دهم."
    )
    try:
        matches, web_results = await find_web_related_models(bd, kw)
        if not matches:
            await status.edit_text(
                f"⚠️ در جستجوی وب، مدل مرتبطی برای <b>{html_entities.escape(kw)}</b> "
                f"که در دیتابیس با بک‌گراند <b>{html_entities.escape(bd)}</b> وجود داشته باشد پیدا نشد.\n\n"
                "نکته: موضوع دیگر مستقیماً با LIKE داخل دیتابیس جستجو نمی‌شود؛ اول وب بررسی می‌شود."
            )
            await message.answer("منوی اصلی:", reply_markup=build_main_menu_kb())
            return

        # حالا فقط برای مدل‌های وب‌تأییدشده، آیتم‌های همان بک‌گراند را از DB می‌گیریم.
        rows: list[tuple[str, str, int]] = []
        for pattern, _score in matches:
            rows.extend(await db_get_items_for_backdrop_pattern(bd, pattern, limit=500))
        # حذف رکوردهای تکراری و حفظ ترتیب امتیاز وب.
        allowed = {p for p, _ in matches}
        dedup: dict[tuple[str, str, int], None] = {}
        for row in rows:
            if row[0] in allowed:
                dedup[row] = None
        rows = list(dedup.keys())
        score_map = dict(matches)
        rows.sort(key=lambda r: (-score_map.get(r[0], 0.0), r[0].lower(), r[1], r[2]))

        models = len({r[0] for r in rows})
        header = (
            f"✅ <b>جستجوی وب + تطبیق NFT</b>\n"
            f"🌐 موضوع: <b>{html_entities.escape(kw)}</b>\n"
            f"🎨 بک‌گراند: <b>{html_entities.escape(bd)}</b>\n"
            f"📦 {models} مدل | {len(rows)} آیتم\n\n"
        )
        chunks = format_collection_results(rows)
        await status.edit_text(header + (chunks[0] if chunks else "⚠️ نتیجه‌ای پیدا نشد."))
        for ch in chunks[1:]:
            await message.answer(ch)

        # برای اولین مدل، اگر نمونه‌ی دقیق model+backdrop موجود باشد کارت تصویری
        # با همان بک‌گراند ساخته می‌شود؛ یعنی عکس خروجی با انتخاب کاربر هماهنگ است.
        first_pattern = matches[0][0]
        sample_rows = [r for r in rows if r[0] == first_pattern]
        if sample_rows:
            base_link = sample_rows[0][1]
            try:
                cards = await build_nft_result_cards(
                    base_link,
                ...
                if cards:
                    pattern_name, card_bytes = cards[0]
                    await message.answer_photo(
                        BufferedInputFile(card_bytes, filename=f"{RE_SAFE_FILENAME.sub('_', pattern_name)}.png"),
                        caption=(
                            f"🪄 <b>مدل مرتبط با موضوع</b>\n"
                            f"🧩 مدل: <b>{html_entities.escape(pattern_name)}</b>\n"
                            f"🎨 بک‌گراند: <b>{html_entities.escape(bd)}</b>"
                        ),
                    )
            except Exception:
                log.exception("ساخت کارت وب‌محور برای نتیجه اول ناموفق بود")

        await message.answer("منوی اصلی:", reply_markup=build_main_menu_kb())
    except Exception:
        log.exception("خطا در جستجوی وب برای موضوع %r", kw)
        await status.edit_text(
            "❌ جستجوی وب موقتاً با خطا مواجه شد.\n"
            "تنظیمات WEB_SEARCH_ENABLED و اتصال اینترنت سرور را بررسی کن."
        )
        await message.answer("منوی اصلی:", reply_markup=build_main_menu_kb())
    finally:
        await state.clear()
...
def build_charge_packages_kb() -> InlineKeyboardMarkup:
    ...
    for ton in TON_PACKAGE_OPTIONS:
        kb.button(text=f"💎 {ton} TON  →  🤖 {ton * SEARCHES_PER_GRAM} اعتبار", callback_data=f"charge:pkg:{ton}")
    kb.adjust(2)
    kb.row(InlineKeyboardButton(text="🔙 منوی اصلی", callback_data="menu:home"))
    return kb.as_markup()
...
def build_charge_selected_kb(ton: int) -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton(text="💎 پرداخت خودکار با Tonkeeper", callback_data=f"charge:auto:{ton}"))
    kb.row(InlineKeyboardButton(text="🧾 پرداخت دستی + ارسال رسید", callback_data=f"charge:manual:{ton}"))
...
async def menu_charge(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "💳 <b>افزایش موجودی</b>\n\n"
        f"هر <b>۱ TON</b> = <b>{SEARCHES_PER_GRAM}</b> اعتبار استفاده از ربات.\n"
        "مقدار TON و اعتبار به‌صورت خودکار به هم وصل هستند تا اشتباه نشود.\n\n"
        "💎 مقدار موردنظر را انتخاب کن:",
        reply_markup=build_charge_packages_kb(),
    )
...
async def charge_pick_package(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "💳 <b>جزئیات شارژ</b>\n\n"
        f"🔹 <b>{ton} TON</b> پرداخت می‌کنی\n"
        f"🤖 <b>{ton * SEARCHES_PER_GRAM} اعتبار</b> دریافت می‌کنی\n\n"
        "یکی از روش‌های پرداخت را انتخاب کن:",
        reply_markup=build_charge_selected_kb(ton),
    )
...
async def charge_auto_start(call: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="💎 پرداخت با Tonkeeper", url=tonkeeper_url)
    ...
    await call.answer()
    await call.message.answer(
        "💎 <b>پرداخت خودکار TON</b>\n\n"
        f"💰 مبلغ: <b>{ton} TON</b>\n"
        f"🤖 اعتبار پس از تأیید: <b>{credits}</b>\n"
        f"🧾 کد پرداخت: <code>{comment}</code>\n\n"
        "روی دکمه پرداخت بزن، در Tonkeeper تراکنش را تأیید کن و برگرد.\n"
        "ربات تراکنش واقعی را روی شبکه بررسی می‌کند و بعد موجودی را خودکار اضافه می‌کند.",
        reply_markup=kb.as_markup(),
    )
...
async def admin_credit_user(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"چند <b>GRAM</b> واریز شود؟ (هر GRAM = {SEARCHES_PER_GRAM} سرچ)\n"
...
async def admin_credit_amount(message: Message, state: FSMContext):
    ...
    if not uid:
        ...
        return
    searches = grams * SEARCHES_PER_GRAM
