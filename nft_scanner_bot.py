...
def get_ton_package_options() -> tuple[int, ...]:
...
async def admin_packages_start(call: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(call.message,
        ...
        "مقادیر جدید را به‌صورت عددهای صحیح و جداشده با کاما بفرست.\n"
        "مثال: <code>1,2,5,10,25,50</code>\n\n"
...
async def admin_packages_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ فقط عدد صحیح بین 1 تا 10000 بفرست؛ مثال: <code>1,2,5,10,20,50</code>")
        return
    if not values or len(values) > 10:
        await message.answer("❌ باید بین 1 تا 10 مبلغ وارد کنی.")
        return
    values.sort()
    await db_set_setting("ton_packages", ",".join(map(str, values)))
    await state.clear()
    await message.answer("✅ مبالغ TON پرداختی ذخیره شد: " + ", ".join(f"{x} TON" for x in values), reply_markup=build_pricing_keyboard())
