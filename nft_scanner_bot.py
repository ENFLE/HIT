...
def get_ton_package_options() -> tuple[float, ...]:
    ...
    for part in str(raw).replace("،", ",").split(","):
        ...
        try:
            ...
            continue
        if 1 <= value <= 10000 and value not in values:
            values.append(value)
...
async def admin_packages_start(call: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(call.message,
        ...
        "مثال: <code>0.5,1,2.5,5,10,25,50</code>\n\n"
        "حداکثر ۱۰ مقدار و هر مقدار بین 1 تا 10000 TON.",
...
async def admin_packages_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ فقط عدد (اعشار هم مجاز است) بین 1 تا 10000 بفرست؛ مثال: <code>0.5,1,2.5,10,20,50</code>")
        return
